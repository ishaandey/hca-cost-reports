...
knitr::opts_chunk$set(include = F, message = F, warning = F,
                      echo=F, eval=T, comment=NA, cache=F #fig.height=4
                      )
```
...
```{r, include=T}
cr %>% filter(provider_number == 490009, year %in% 2016:2020) %>% 
...
We define the Tampa-St.Petersburg market as the four adjacent counties: Pasco, Hillsborough, Pinellas, and Hernando. Of the 41 hospitals in the region, 8 are HCA-owned (20%) in 2019.

## Statistical Modelling

We first model Medicare discharges in the region, controlling for year effects using a nonparametric robust least-squares design. We find a significant coefficient associated with HCA ownership.
...
We find no such effect when modelling Medicare volumes.
